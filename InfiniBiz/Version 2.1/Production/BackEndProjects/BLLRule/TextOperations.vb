...
#Region ".........................BizPlan Web Methods"
    ...
    Public Shared Function GetPlanRights(ByRef connData As ConnectionData, ByVal EmployeeID As String, ByVal IsMerchant As Boolean, Optional ByVal ServiceID As String = "15") As DataTable
        Dim _Mstatus As Integer
        ...
        cmd.AddParameter("@UserID", EmployeeID)
        cmd.AddParameter("@ServiceID", ServiceID)
        cmd.AddParameter("@MIdentity", connData.CustomerID)
        cmd.AddParameter("@IsMerchant", _Mstatus)

        cmd.CmdText = "BPL_FETCHUSERSRIGHTS"
...
#Region " PlanoutLine"
    Public Shared Function GetOutLineHeadings(ByRef connData As ConnectionData, ByRef currentPlan As Infinilogic.BusinessPlan.BLL.BusinessPlan) As DataSet

        Dim cmd As New CommandData(connData.CustomerID)
        Dim ds As DataSet
        Try
            cmd.BeginTransaction()
            cmd.CmdText = "BPL_GetOutLineHeadings"
            cmd.AddParameter("@PlanID", currentPlan.PlanID)
            ' 0 for Common : 1 for Products : 2 for Services : 3 for Both
            cmd.AddParameter("@businessGoods", GetBusinessGoods(currentPlan.BusinessGoods))
            '0 for Common : 1 for Ongoing : 2 for Startup Plans
    ...
    Public Shared Function RestorePlanTextHeadings(ByRef connData As ConnectionData, ByRef currentPlan As InfiniLogic.BusinessPlan.BLL.BusinessPlan, ByVal id As Integer) As String
