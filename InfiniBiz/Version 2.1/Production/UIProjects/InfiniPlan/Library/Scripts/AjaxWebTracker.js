...
function stateChangeHandlerGetDetailOrder()
{ 
 	if(xmlHttp.readyState == 4 )
	{ 
 		var str = xmlHttp.responseText
	 	...
	 	document.getElementById('lblHead').innerHTML = "Order's Detail";
...
// Test
function getDetails_Customer(CustomerID )
{
// debugger;
	var url = requestURL + 'CustomerID_' + CustomerID  ; 
	xmlHttp = GetXmlHttpObject(stateChangeHandlerGetDetail_Customer); 
	xmlHttp_Get(xmlHttp, url);
}			
function stateChangeHandlerGetDetail_Customer()
{ 
	if(xmlHttp.readyState == 4 )
				{ 
		 		var str1 = xmlHttp.responseText
		 	 	document.getElementById('DetailDiv').innerHTML = str1
		 	 	document.getElementById('lblHead').innerHTML = "Customer Navigation Detail";
		    	 }
} 
//test
...
// Referrer
function getDetails_CustomerReferrer(CustomerID )
{
// debugger;+
 
	var urlReferrer = requestURL + 'CustomerIDReferrer_' + CustomerID  ; 
	 
	xmlHttp = GetXmlHttpObject(stateChangeHandlerGetDetail_Customer); 
	xmlHttp_Get(xmlHttp, urlReferrer);
}			
function stateChangeHandlerGetDetail_Customer()
{ 
	if(xmlHttp.readyState == 4 )
...
//Referrer
	  ...
	  // Activity Test
function getDetails_CustomerActivity(CustomerID )
{
 //debugger;
	var url = requestURL + 'CustomerActivityID_' + CustomerID  ;
	
	xmlHttp = GetXmlHttpObject(stateChangeHandlerGetDetail_CustomerActivity); 
	xmlHttp_Get(xmlHttp, url);
}			
function stateChangeHandlerGetDetail_CustomerActivity()
{ 
	if(xmlHttp.readyState == 4 )
				{ 
		 		var str_cust = xmlHttp.responseText
		 		
		 	 	document.getElementById('DetailDiv').innerHTML = str_cust
		 	 	document.getElementById('lblHead').innerHTML = "Customer Navigation Detail";
		    	 }
} 
//Activity test
...
{
	//debugger;
	if (document.getElementById('onlineVisiter') == null)
	{
	}
	else
	{
