...
#End Region
    ...
    Private Sub CreatePDF(ByVal OrderID As Integer, ByVal InvoiceNo As Integer, ByVal chInvoice As Char)
        ...
        If (isInvoiceData) Then
            ...
            With invoice_data
                Do
                    ...
                    Try
                        templateHTML = Replace(templateHTML, "<!--IMAGE_SRC-->", "http://www.accountscentre.com/PDF/DrawPdfLogo.aspx?customerid=" & CustomerID & "&Currenttime=" & Now.ToString)
                    Catch ex As Exception
        ...
        Dim strtempfilename As String = Server.MapPath("\PDF\tempM_" & CustomerID & ".pdf")
        ...
        If pdffiledata.Length > 1 Then
            With Response
                Try
                    ...
                    'Date:          Aug 07, 2007
                    If chInvoice = "N" Then
                        .AppendHeader("content-disposition", "attachment; filename=" + ("Order_Detail_" & InvoiceNo & ".pdf"))
                    Else
                        .AppendHeader("content-disposition", "attachment; filename=" + ("Invoice_Detail_" & InvoiceNo & ".pdf"))
