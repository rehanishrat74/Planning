...
//----------------------------------------------------------------------
class  FlashPackages.EnterpriseActualROI {
	...
	function EnterpriseActualROI(  Customerid,dateStart, dateEnd,LastStartDate ,LastEndDate, getCurrency,meterName,meterType ) {
		//trace("*** MerchantActualSpeedoMeter ***")
		  var stockservice = new WebService("http://accounts.infinibiz.com/MeterTesting/MeterViewService.asmx?wsdl"); 	// WebService Object 
				...
				//   var myObj = stockservice.LoadEnterpriseMeters(  "115161", "may 26 2003  00:00:00.000", "may 25 2004 23:59:59.000" , "may 26 2002  00:00:00.000" , "may 25 2003 23:59:59.000" , "A","ROI" );
			var InputType 
		...
		//@-------------------------------------------------------------------
			...
			_root.ActualPrice_txt.text = productActualValue +" "+ "%";
		...
		//@-------------------------------------------------------------------
			...
			_root.hover_mc.onRollOver = function() {
				...
				_root.tooltip_mc.toolTipActualValue_txt.text = this.productActualValue+" "+ "%";		//Tooltip Actual Value Box  
				_root.tooltip_mc.toolTipEstimatedValue_txt.text = this.productEstimatedValue+" "+ "%";//Tooltip Estimated Value Box			startDrag(_root.tooltip_mc,true); 
	...
	function dynamicCircle(moveFrom,moveTo,numbersOfPie, actualPrice, estimatedPrice){
		...
		for(var i=-140;i<140;i+=3){
			...
			if(_root.needle_mc._rotation >= Math.round(moveFrom)){
				...
				if (InputType == "Sales" || InputType == "Profit" || "ROI")
			{
				trace('c');
			colorObj.setRGB(0x000000)	//  black Color; 
