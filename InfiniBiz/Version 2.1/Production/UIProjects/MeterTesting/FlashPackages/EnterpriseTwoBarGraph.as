...
//imports WebService 
//@---------------------------------------------------
class FlashPackages.EnterpriseTwoBarGraph {
	private var estamitedBarMovement;
	private var percentage;
	function EnterpriseTwoBarGraph(Customerid, dateStart, dateEnd, LastStartDate, LastEndDate, getCurrency, meterName, meterType) {
		...
		//  var myObj = stockservice.LoadEnterpriseMeters("115161", "may 7 2007  00:00:00.000", "may 6 2008 23:59:59.000", "may 7 2006  00:00:00.000", "may 6 2007 23:59:59.000", "A", "Sales");
		 var InputType // =  
		// meterType
		myObj.onResult = function(result) {
			...
			var productName = meterName;
			// Name; // productsList[0].attributes.ProductName;// Sets Product Name
			...
			if (ActualBarValue>EstimatedBarValue && ActualBarValue != 0 && EstimatedBarValue != 0) {
				...
				if (InputType == "Sales" || InputType == "Profit") {
					trace('a');
					setColor(_root.actualBar_mc,  0x00000);
					// black ff3300 
					setColor(_root.actEstBar_mc, 0x66cc00  );
					// Green
					setColor(_root.estimatedBar_mc, 0x66cc00);
					// Green
				...
				if (InputType == "Sales" || InputType == "Profit") {
					trace('a');
					setColor(_root.actualBar_mc, 0xff0000);
					//Red
				...
				if (InputType == "Sales" || InputType == "Profit") {
					...
					setColor(_root.estimatedBar_mc, 0xFFFFFF);
					//red 
					setColor(_root.actEstBar_mc, 0xffcc01);
					// ornage
				}
				new Tween(_root.actualBar_mc, "_yscale", mx.transitions.easing.Regular.easeOut, 0, -100, 1, true);
				new Tween(_root.estimatedBar_mc, "_yscale", mx.transitions.easing.Regular.easeOut, 0, -5, 1, true);
