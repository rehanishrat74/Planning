...
//imports WebService 
//@---------------------------------------------------
class FlashPackages.EnterpriseTwoBarROI {
	private var estamitedBarMovement;
	private var percentage;
	function EnterpriseTwoBarROI(Customerid, dateStart, dateEnd, LastStartDate, LastEndDate, getCurrency, meterName, meterType) {
		...
		myObj.onResult = function(result) {
			...
			if (ActualBarValue>EstimatedBarValue && ActualBarValue != 0 && EstimatedBarValue != 0) {
				...
				if (InputType == "Sales" || InputType == "Profit" ) {
					trace('a');
					setColor(_root.EstActBar, 0xffcc00 );
					// yellow  
					setColor(_root.estimatedBar_mc, 0x66cc00);
					// Green
			...
			_root.hover_mc.onRollOver = function() {
				...
				_root.tolltip_mc.Caption_ActualPrice.text = ActualBarValue+" "+ "%";
				_root.tolltip_mc.Caption_EstimatedPrice.text = EstimatedBarValue+" "+"%";
