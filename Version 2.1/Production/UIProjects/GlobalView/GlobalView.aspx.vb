...
#Region "Event Handlres"
    ...
    Public Sub lnkCompany_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        With dgridSummaryInformation
            sRegID = .SelectedItem.Cells(10).Text '  CType(.Items(0).FindControl("RegID"), HtmlInputHidden)
            sRegUserID = .SelectedItem.Cells(6).Text ' CType(.Items(0).FindControl("RegUserID"), HtmlInputHidden)
            sGVCompName = .SelectedItem.Cells(11).Text
            'CType(.Items(0).FindControl("GVCompName"), HtmlInputHidden)
            sGVCompNumber = .SelectedItem.Cells(9).Text 'CType(.Items(0).FindControl("GVCompNumber"), HtmlInputHidden)
        End With
        Dim sRedirectURL As String
        Dim sCompanyName As String
    ...
    Private Sub dgridSummaryInformation_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dgridSummaryInformation.ItemDataBound
        Select Case e.Item.ItemType
            Case ListItemType.Item, ListItemType.AlternatingItem
                If e.Item.Cells(3).Text = "DUE" Then
                    e.Item.Cells(3).ForeColor = Color.DarkOrange
                ElseIf e.Item.Cells(3).Text = "OVER DUE" Then
                ...
                If IsNumeric(e.Item.Cells(10).Text) = False Then
                    CType(e.Item.Cells(5).Controls(0), LinkButton).Text = "Register"
                End If
...
#End Region
    ...
    Private Function FindInGrid(ByRef grd As System.Web.UI.WebControls.DataGrid, ByVal sCompanyName As String) As Integer
        ...
        For Each gi In grd.Items
            Dim d1 As String = Trim(gi.Cells(11).Text.ToUpper)
            Dim d2 As String = Trim(sCompanyName.ToUpper)

            If Trim(gi.Cells(11).Text.ToUpper) = Trim(sCompanyName.ToUpper) Then
